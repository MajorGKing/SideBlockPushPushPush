...
namespace Server.Data
{
    ...
    public class BuddyGachaDataLoader : ILoader<string, BuddyGachaData>
    {
        ...
        public bool Validate()
        {
            DataManager.commonBuddies = new List<string>();
            DataManager.rareBuddies = new List<string>();
            DataManager.epicBuddies = new List<string>();
            DataManager.uniqueBuddies = new List<string>();
            DataManager.legendBuddies = new List<string>();

            foreach (var buddy in buddyGachaList)
            {
                if (buddy.Rarity == Define.ERarityType.Common)
                {
                    DataManager.commonBuddies.Add(buddy.GachaItem);
                }
                else if (buddy.Rarity == Define.ERarityType.Rare)
                {
                    DataManager.rareBuddies.Add(buddy.GachaItem);
                }
                else if (buddy.Rarity == Define.ERarityType.Epic)
                {
                    DataManager.epicBuddies.Add(buddy.GachaItem);
                }
                else if (buddy.Rarity == Define.ERarityType.Unique)
                {
                    DataManager.uniqueBuddies.Add(buddy.GachaItem);
                }
                else if (buddy.Rarity == Define.ERarityType.Legend)
                {
                    DataManager.legendBuddies.Add(buddy.GachaItem);
                }
            }
